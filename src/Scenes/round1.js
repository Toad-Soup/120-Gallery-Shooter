...
class round1 extends Phaser.Scene {
    ...
    //create the duck sprite ************************************************************
        ...
        for(let i = 0; i < 3; i++){
            let bulletHole = this.add.sprite(-100, 380, "objectParts", "shot_yellow_small.png");
            my.sprite.holes.push(bulletHole);//visible = false;
        }
    ...
    update() {
        ...
        for (let duck of my.sprite.ducks1) {
        ...
        //moves the sticks
        for (let stick of my.sprite.sticks1) {
        ...
        //my.sprite.water = my.sprite.water.filter((water) => water.y > -(water.displayHeight/2));
        ...
        /*
        if(this.fireResetDelay) {
            if (this.time.now - this.fireDelayTimer >= this.fireDelayDuration) {
                this.fireActiveDelay = false;
                // Reset HUD bullet textures
        ...
        */
        ...
        for(let bullet of my.sprite.bullet){
            ...
            let allShot = my.sprite.ducks1.every(duck => duck.hit);
            //console.log(my.sprite.ducks1.map(d => d.wasShot));

            if(allShot === true){
                allShot = false;
                for (let duck of my.sprite.ducks1) {
                    duck.y -= duck.speed * duck.direction;
                    //duck.visible = false;
                    //duck.x = -1000; // Move offscreen
                }
                for(let stick of my.sprite.sticks1){
                    stick.y -= stick.speed * stick.direction;
                    //stick.visible = false;
                    //stick.x = -1000;
                }

                //set up the second round!
                //this.scene.stop("round1");
